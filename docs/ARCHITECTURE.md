...
- New projects get root `AGENTS.md`, `README.md`, `.gitignore`, and `scripts/sz.py` (from `templates/sz.py`), plus project docs under `docs/` (or `SCAFFOLD_DIR_NAME`), copied from `~/.config/new-proj/templates/` (refreshed on every `./install.sh`).
...
quick-project-start/
  ...
  templates/        # Scaffold templates (docs + sz.py) synced by install.sh
...
| `~/.config/new-proj/templates/` | `AGENTS.md`, `ARCHITECTURE.md`, `KNOWLEDGE.md`, `README.md`, `.gitignore`, `sz.py` — refreshed on every `./install.sh`; doc templates under scaffold dir; `sz.py` copied to project root `scripts/` |
...
flowchart LR
  ...
  subgraph out["new project"]
    root["AGENTS.md README.md .gitignore scripts/sz.py"]
...
- **Stack**: Bash only; no runtime dependencies beyond `git` and optional `gh`. Scaffold utility `scripts/sz.py` uses Python 3 stdlib only; template source is flat `templates/sz.py`.
...
- **`AGENTS.md` and `README.md` at project root** for new projects; `ARCHITECTURE.md` and `KNOWLEDGE.md` under `docs/` (or `SCAFFOLD_DIR_NAME`); `docs/skills/` created empty; `templates/sz.py` copied to root `scripts/sz.py`.
