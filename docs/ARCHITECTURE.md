...
- Pass `--agent-version` from anywhere inside an existing project to print the project's `AGENTS.md` version (last non-empty line, format `AGENTS.md version: X.Y.Z`) and the latest available version from the same template chain as `--agent-upgrade`. Exit 0 when they match, 1 when the project is missing a version, behind, or has no `AGENTS.md`.
- Root `AGENTS.md` ends with `AGENTS.md version: X.Y.Z` on the last line; bump semver whenever the template changes.
