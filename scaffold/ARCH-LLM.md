...
- `ARCH-HUMAN.md`, `ARCH-LLM.md` (added if missing; not overwritten on `--update`)
- `skills/` (empty dir)
...
1. `./install.sh` → `~/.local/bin/quick-proj` + `~/.local/share/quick-proj/lib/`, sync templates + bundled agent files, create/fix `config.env` (`docs` → `scaffold`).
